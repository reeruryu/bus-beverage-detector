...
from bus_beverage_detection.misc import Timer ## 작성한 타이머 관련 코드 import
...
class DetectionNode(Node):
    ...
    def __init__(self):
        ...
        self.ckpoint = torch.load("./src/bus_beverage_detection/SSD_CheckPoint.pth") ## 체크포인트 경로 지정
        self.net = self.ckpoint['model'].cuda() ## SSD_mobilenet 모델 선언후 GPU로 올리기
        self.net.score_thresh=0.3
        #=====        
    ...
    def detect_callback(self, data):
        ...
        for i in range(boxes.size(0)):
            ...
            if 'hold_phone' in label:
            	color = (255, 0, 0)
            elif 'hold_drink' in label:
            	color = (0, 0, 255)
